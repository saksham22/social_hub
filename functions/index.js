...
const timelineRef = admin.firestore().collection('timeline').doc(followerId).collection('timelinePosts');
...
const timelineRef = admin.firestore().collection("timeline").doc(followerId).collection("timelinePosts").where("ownerId","==",userId);
const querySnapshot = await timelineRef.get();
querySnapshot.forEach(doc =>{
if(doc.exists){
doc.ref.delete();
}
...
//1) get all the followers of users who
const userFollowers = admin.firestore().collection('followers').doc(userId).collection('userFollowers');
const querySnapshot = await userFollowers.get()
//2) add new posts to each follower's timeline
querySnapshot.forEach(doc => {
...
exports.onDeletePost = functions.firestore.document("/posts/{userId}/userPosts/{postId}").onDelete(async (snapshot, context) =>{
	const userId = context.params.userId;
    const postId = context.params.postId;
	console.log('deleting from timeline',userId);
